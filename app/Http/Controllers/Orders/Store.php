...
class Store extends BaseController
{
    ...
    public function __invoke(StoreRequest $request)
    {
        ...
        $demandResult = (new Demand)->create(new DemandEntity(
            $orderResult->id,
            new FullNameEntity(
                $demandAddress->last_name,
                $demandAddress->last_name_kana,
                $demandAddress->first_name,
                $demandAddress->first_name_kana,
            ),
            new AddressContentEntity(
                $demandAddress->post_number,
                $demandAddress->prefecture_name,
                $demandAddress->city,
                $demandAddress->block,
                $demandAddress->building,
            ),
            $deliverAddress->phone_number,
            $user->email,
        ));
